...
> **진화 위치**: 석기시대(직접 명령 입력)를 넘어, 이제 **역할 분업과 도메인 특화**의 시대입니다.
> - 청동기 = 에이전트 분업 (Subagent, Agent Teams) — "여러 봇이 역할을 나눈다"
> - 철기 = 도메인 특화 + 자동화 (Hook, MCP) — "각 봇이 전문가가 된다"

참조: `references/evolution-chronicle.md` — 시대 3(청동기)과 시대 4(철기)의 실제 사례
...
진화 시대별 매핑:
```
🗡️ 청동기 기능: Subagent, Agent Teams → 역할 분업
⚔️ 철기 기능:   Hook, MCP → 도메인 특화 + 자동화
💎 다이아 기능:  Skill → LLM 기반 판단 (Block 5에서 심화)
📦 메타 기능:    Plugin → 통합 배포 (졸업 후)
```
