...
> **진화 위치**: 청동기~철기(역할 분업, 도메인 특화)를 마스터했다면, 이제 **다이아몬드시대**입니다.
> 다이아몬드시대 = LLM에게 프롬프트로 행동을 지시하는 단계.
> 마인크래프트에서 `companion_bot.js`가 LLM으로 자연어를 이해하고 목표를 분해했듯이,
> Claude Code에서는 **SKILL.md 하나로 복잡한 작업을 프롬프트로 해결**합니다.

참조: `references/evolution-chronicle.md` — 시대 5(다이아몬드)의 실제 사례
...
→ Block 6(네더라이트시대: RAG + 리서치 결합)으로 이동 안내.
